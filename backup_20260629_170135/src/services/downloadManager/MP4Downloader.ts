...
class MP4Downloader {
  ...
  async start() {
    try {
      ...
      const fileInfo = await LegacyFileSystem.getInfoAsync(videoPath);
  ...
  async resume(savable = null) {
    ...
    if (this.downloadResumable) {
      try {
        ...
        if (result) {
          const resumeFileInfo = await LegacyFileSystem.getInfoAsync(result.uri);
          ...
          if (this.onComplete) {
            this.onComplete({
              filePath: result.uri,
              fileSize: resumeFileInfo.exists ? (resumeFileInfo.size || 0) : 0,
            });
          }
        }
      } catch (error) {
