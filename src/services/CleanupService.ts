...
class CleanupService {
  ...
  async runCleanup() {
    try {
      ...
      const autoDeleteWatchedDays = state.autoDeleteWatchedDays;
      const autoDeleteUnwatchedDays = state.autoDeleteUnwatchedDays;
      ...
      for (const download of downloads) {
        ...
        if (autoDeleteWatchedDays > 0 && download.lastWatchedAt) {
          const watchedDate = new Date(download.lastWatchedAt);
          const daysSinceWatched = (now.getTime() - watchedDate.getTime()) / (1000 * 60 * 60 * 24);

          if (daysSinceWatched >= autoDeleteWatchedDays) {
            shouldDelete = true;
            reason = 'watched';
          }
        }

        if (!shouldDelete && autoDeleteUnwatchedDays > 0 && !download.lastWatchedAt) {
          ...
          if (daysSinceDownload >= autoDeleteUnwatchedDays) {
            shouldDelete = true;
            reason = 'unwatched';
          }
        }
