...
class DownloadManager {
  ...
  async canDownload() {
    ...
    const wifiOnlyDownload = state.wifiOnlyDownload;
    return networkMonitor.canDownload(wifiOnlyDownload);
  }
  ...
  async processQueue() {
    ...
    try {
      ...
      // Get max concurrent downloads from Zustand
      const state = useAppStore.getState();
      const maxConcurrentDownloads = state.maxConcurrentDownloads;
      
      const currentlyProcessing = this.activeDownloads.size + this.pendingFetches.size;
      const availableSlots = maxConcurrentDownloads - currentlyProcessing;
  ...
  async fetchAndStartDownload(entry: any) {
    try {
      ...
      if (entry.mediaType === 'tv') {
        fetchUrl = ${fluxSource.baseUrl}?tmdbId=&season=&episode=;
      } else {
        fetchUrl = ${fluxSource.baseUrl}?tmdbId=;
      }
