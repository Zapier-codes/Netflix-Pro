...
import { extractLiveStreamM3U8 } from '../api/ApiService';
import { getActiveStreamSources } from '../services/unified/providers/vidsrc/VidSrcProvider';
import { getCachedStreamUrl, saveStreamUrl, clearSpecificStreamFromCache } from '../utils/storage';
...
export const useStreamExtraction = ({
  ...
  const setupLiveStreamExtraction = useCallback(
    (isMountedRef) => {
      ...
      extractLiveStreamM3U8(
        ...
        (err, sourceName) => {
          if (!isMountedRef.current) return;
          console.error(
            `[useStreamExtraction] Live stream extraction error from ${sourceName}:`,
            err.message
          );
          onError({
            message:
              'Failed to load live stream. The stream may have ended or is no longer available.',
  ...
  const setupStreamExtraction = useCallback(
    (isMountedRef) => {
      if (!isMountedRef.current) return;
      setCurrentAttemptingSource(null);
      ...
      extractM3U8Stream(
        ...
        (finalError) => {
          ...
          if (currentEpisodeAirDate && isFutureDate(currentEpisodeAirDate)) {
            ...
            onError({
              message: `All sources failed: ${finalError.message || 'Could not find a playable stream.'}`,
            });
          }
          setStreamExtractionComplete(true);
          setManualWebViewVisible(false);
      ...
      if (isOffline && offlineFilePath) {
        ...
        if (fileInfo.exists) {
          ...
          try {
            ...
            console.warn(
              '[useStreamExtraction] Failed to clean up missing download entry:',
              cleanupErr
            );
          }
        }
      ...
      if (hasDownload && downloadEntry.filePath) {
          ...
          if (fileInfo.exists) {
            if (fileInfo.isDirectory) {
              ...
              const videoFile = files.find(
                (file) => file.endsWith('.mp4') || file.endsWith('.m3u8')
              );
        ...
        if (videoFilePath) {
          ...
          try {
            ...
            console.error(
              '[useStreamExtraction] Downloaded file playback error:',
              playerErr.message
            );
          }
  ...
  const openChangeSourceModal = useCallback(
    async (isInitialLoading) => {
      ...
      if (player.isPlaying) {
        try {
          ...
          console.warn('Error pausing video before opening source modal:', e);
        }
      }
  ...
  const handleSelectSourceFromModal = useCallback(
    async (selectedSourceInfo, position, isUnmounting, setShowControls) => {
      ...
      if (player.isPlaying) {
        try {
          ...
          console.warn('Error pausing video on source select:', e);
        }
      }
      ...
      const onSourceErrorCallback = (err, sourceName) => {
        ...
        console.warn(
          `[useStreamExtraction] SpecificSource: Error from source ${sourceName}: ${err.message}`
        );
        setSourceAttemptStatus((prev) => ({ ...prev, [sourceName]: 'failed' }));
